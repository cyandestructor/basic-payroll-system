...
IF (EXISTS(SELECT name FROM sysobjects WHERE name = 'LASTVACATIONDATE' AND type = 'FN'))
	DROP FUNCTION LASTVACATIONDATE;
GO

CREATE FUNCTION LASTVACATIONDATE (
	@ID_Empleado	INT
)
RETURNS DATE
AS
	BEGIN
		DECLARE @LastVacationDate DATE;

		SET @LastVacationDate = (
			SELECT TOP 1
				Fecha_Solicitud
			FROM
				Incidencia
			WHERE
				ID_Solicitante = @ID_Empleado AND Desc_Inciden LIKE '%Vacaciones%'
			ORDER BY
...
IF (EXISTS(SELECT name FROM sysobjects WHERE name = 'NUMBERTOWORDS' AND type = 'FN'))
	DROP FUNCTION NUMBERTOWORDS;
GO

CREATE FUNCTION NUMBERTOWORDS
(
    @Numero             Decimal(18,2)
)
RETURNS Varchar(180)
